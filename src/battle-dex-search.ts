...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	'ssdlc1' | 'ssdlc1doubles' | 'predlc' | 'predlcdoubles' | 'predlcnatdex' | 'stadium' | 'lc' | 'spestd' | null = null;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | PokemonSet = '' as ID) {
		...
		if (format.includes('special')) {
			if (format.includes('standard')) {
				this.dex = Dex.mod('gen9spestd' as ID);
				this.formatType = 'spestd';
			}
		}
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		// * Custom format
		if (this.formatType === 'spestd') table = table['gen9spestd'];
	...
	getTier(pokemon: Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			// * Custom format
			this.formatType === 'spestd' ? `gen${gen}spestd` :
			`gen${gen}`;
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		} else if (this.formatType === 'spestd') {
			// * Custom format
			table = table['gen' + dex.gen + 'spestd'];
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		// * Custom format
		if (this.formatType === 'spestd') lsetTable = lsetTable['gen9spestd'];
